...
const (
	// FileFormatRaw represents the raw disk image format. This format has the advantage
	// of being simple and easily exportable to all other emulators. If your file
	// system supports holes (for example in ext2 or ext3 on Linux or NTFS on
	...
	// Use it to have smaller images (useful if your filesystem does not support holes,
	// for example on Windows), zlib based compression and support of multiple VM snapshots.
	FileFormatQCOW2 FileFormat = "qcow2"

	// FileFormatQED represents the old QEMU image format with support for backing files
	// and compact image files (when your filesystem or transport medium does not
	// support holes).
	...
	// FileFormatLUKS represents the LUKS v1 encryption format, compatible with Linux
	// dm-crypt/cryptsetup.
...
const (
	...
	// ReadOnlyFormatCloop represents the Linux Compressed Loop image, useful only
	// to reuse directly compressed CD-ROM images present for example in the
	// Knoppix CD-ROMs.
